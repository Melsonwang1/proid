// Fixed Supabase Authentication Module with Custom Users Table
...
async function signupUser(userData) {
    try {
        ...
        // If auth signup successful and user was created, add to custom users table
        if (authData.user) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (insertError) {
                        ...
                        // Try with service role approach
                        try {
                            ...
                            if (!response.ok) {
                                console.error('Direct API insert also failed:', await response.text());
                            } else {
                                console.log('User record created successfully via direct API');
                            }
                        } catch (apiError) {
                            console.error('Direct API insert failed:', apiError);
                        }
                    } else {
                        console.log('User record created successfully in custom table');
                    }
                } catch (tableError) {
                    console.error('Custom table insert failed:', tableError);
                }
            }, 1000); // Wait 1 second for auth context to establish
        }
